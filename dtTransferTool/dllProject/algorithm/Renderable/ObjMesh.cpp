...
ObjMesh::ObjMesh(const MeshInfo* info) {
	for (size_t i = 0; i < info->vertexCount; i++)
	{
		vertex_list.push_back(ldp::Float3(info->vertexes[3 * i], info->vertexes[3 * i + 1], info->vertexes[3 * i + 2]));
	}
	for (size_t j = 0; j < info->faceCount; j++)
	{
		obj_face face;
		face.vertex_count = 3;
		face.vertex_index[0] = info->indices[3 * j];
		face.vertex_index[1] = info->indices[3 * j + 1];
		face.vertex_index[2] = info->indices[3 * j + 2];
