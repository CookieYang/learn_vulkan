...
/// TODO: we should manually specify anchor points, instead of just fixing the 0th vertices.
	...
	public:
		...
		int init(int nTriangles, const Int3* pTriangles, int nVertices,
		...
		int transfer(const std::vector<Float3>& srcVerts1, std::vector<Float3>& tarVerts1);

		const char* getErrString()const;
	protected:
		void clear();
		void findAnchorPoints(const std::vector<int>& fixVertIds);
		...
		void setup_pairMat();
		void setup_pairRhs(const std::vector<Float3>& tarVerts0);
	...
	private:
		...
		std::vector<Int2> m_vertPairs;		// pairs of vertices that should be connected
		...
		SpMat m_pairMat;					// for pair-connection points
		SpMat m_pairMatT;					// for pair-connection points
		Vec m_pairRhs;						// for pair-connection points
